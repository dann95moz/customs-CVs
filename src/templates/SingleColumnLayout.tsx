import React from 'react';
import { CVSlotMap } from './types';
import { ThemeId } from '../types/cv';
import { 
  HeaderSlot, 
  SummarySlot, 
  SkillsSlot, 
  ExperienceSlot, 
  EducationSlot, 
  LanguagesSlot, 
  GenericSlot 
} from '../components/slots';

export interface SingleColumnLayoutProps {
  slots: CVSlotMap;
  theme: ThemeId;
  templateClass: string;
  skillsVariant?: 'pills' | 'inline';
}

/**
 * Reusable layout engine for all single-column CV templates.
 * Enforces DRY and Open/Closed principles across themes.
 */
export const SingleColumnLayout: React.FC<SingleColumnLayoutProps> = ({
  slots,
  theme,
  templateClass,
  skillsVariant = 'inline'
}) => {
  return (
    <div className={`theme-${theme} ${templateClass}`}>
      <div className="cv-container">
        {/* Header Slot */}
        <HeaderSlot data={slots.header} />

        {/* Linear Content Flow */}
        <main className="cv-body">
          {slots.summary && (
            <SummarySlot data={slots.summary} />
          )}

          {slots.skills && (
            <SkillsSlot data={slots.skills} variant={skillsVariant} />
          )}

          {slots.experience && (
            <ExperienceSlot data={slots.experience} />
          )}

          {slots.projects && (
            <ExperienceSlot data={slots.projects} />
          )}

          {/* Bottom Balanced Row: Education on Left, Languages on Right */}
          {(slots.education || slots.languages) && (
            <div 
              className="cv-bottom-grid" 
              style={{ 
                display: 'grid', 
                gridTemplateColumns: slots.languages && slots.education ? '1.75fr 1fr' : '1fr', 
                gap: '18px' 
              }}
            >
              {slots.education && (
                <EducationSlot data={slots.education} />
              )}
              {slots.languages && (
                <LanguagesSlot data={slots.languages} />
              )}
            </div>
          )}

          {slots.genericSections.map(sec => (
            <GenericSlot key={sec.id} data={sec} />
          ))}
        </main>
      </div>
    </div>
  );
};
