...
export const InterviewPrepTab: React.FC<InterviewPrepTabProps> = ({
  ...
  const handleCopyAll = () => {
    ...
    const allText = `# Interview Preparation for ${targetRole} at ${companyName}\n\n` +
      `## Coaching Tips:\n` +
      prepResult.overallTips.map((tip) => `- ${tip}`).join('\n') +
      `\n\n## Questions & STAR Answering Strategies:\n\n` +
      prepResult.questions
        .map(
          (q, idx) =>
            ...
            `  * **Action:** ${q.starStrategy.action}\n` +
            `  * **Result:** ${q.starStrategy.result}\n` +
            `* **Speaking Points:** ${q.sampleAnswerOutline || 'N/A'}\n`
        )
        .join('\n');

    navigator.clipboard.writeText(allText);
    setSnackbar(t('audit:interview.copiedAll', 'Full interview preparation kit copied to clipboard!'));
  };
